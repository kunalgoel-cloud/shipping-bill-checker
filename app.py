...
with st.sidebar:
    ...
    st.write(
        "Upload the agreement workbook (zone-to-zone rate matrix + zone "
        "mapping + charges). Don't have one yet? Grab the template below — "
        "it comes pre-filled with a sample rate card you can overwrite."
    )
    ...
    if st.session_state.agreement is None:
        st.info("No agreement loaded yet. Rate checks will be skipped until one is uploaded.")
...
if st.button("\u2705 Run Check", type="primary", use_container_width=True):
    if st.session_state.agreement is None:
        st.error("Upload a commercial agreement first (left sidebar).")
    else:
